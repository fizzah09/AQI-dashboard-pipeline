...
def load_config(rel_path: str = "config/config.yaml") -> Dict[str, Any]:
    ...
    # repo root = two levels up from this file (src/config/__init__.py)
    root = Path(__file__).resolve().parents[2]
    
    # Search for .env: first in project root, then parent (AQI-DASHBOARD)
    env_path = root / ".env"
    ...
    if env_path.exists():
        ...
        try:
            ...
            # Manual fallback if python-dotenv not present
            try:
